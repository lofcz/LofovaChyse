...
namespace DataAccess.Dao
{
    public class BookSekceDao : DaoBase<BookSekce>
    {
        ...
        {
            return session.CreateCriteria<BookSekce>().AddOrder(Order.Asc("Id")).Add(Restrictions.Like("ParentId", id)).List<BookSekce>();
        }

        public IList<BookSekce> GetCategoriesDebug(int? id)
        {
            return session.CreateCriteria<BookSekce>().AddOrder(Order.Desc("Id")).Add(Restrictions.Like("ParentId", id)).List<BookSekce>();
        }
